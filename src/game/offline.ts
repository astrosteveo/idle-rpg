...
import { itemScore, makeItem, rollRarity, sumStats } from './loot'
...
export interface OfflineReport {
  ...
  levelFrom: number
  levelTo: number
  /** The best few drops, already sorted by score. */
  items: Item[]
  soldCount: number
  soldGold: number
}
 ...
 */
export function runOfflineLedger(save: SaveV1, nowMs: number, rand: Rng): OfflineReport | null {
  ...
  while (remaining > 0) {
    ...
    const stats = deriveStats(level, base, gear)
    const kills = (killsPerHour(stats, kind, groundLevel) / 3600) * slice
    if (kills <= 0) continue

    const scale = rewardScale(level, groundLevel)
    killsFloat += kills
    gold += kills * goldPerKill * scale
  ...
  const expectedDrops = Math.round(kills * type.dropChance * finalScale)
  ...
  return {
    ...
    kills,
    eliteKills: Math.round(kills * eliteShare),
